...
namespace ConsolePNG
{
    ...
    class Program
    {
        ...
        unsafe static Bitmap MergeTwo8bpp(Bitmap firstImage, Bitmap secondImage)
        {
            if (firstImage == null)
            {
        ...
        public static Bitmap MergeTwo32Argb(Bitmap firstImage, Bitmap secondImage)
        {
            if (firstImage == null)
            {
                throw new ArgumentNullException("firstImage");
        ...
        {
            byte[] memblock;
            memblock = File.ReadAllBytes("test.gba");
            ...
            while (Num <= 256)
            {
                ...
                for (; Num <= 256; Num++)
                {
                    ...
                    {
                        Path += "0";
                        Path += ConvertHexToChar(Num / 10);
                    }
                    else
                    {
                        Path += ConvertHexToChar(Num / 100);
                        Path += ConvertHexToChar((Num / 10) % 10);
                    }
                    Path += Num % 10;
                    if (File.Exists(Path + ".png"))
                    {
                        int Back = 0, Enemynum = Num;
                        if (File.Exists(Path + "Back.png"))
                            Back = 1;
                        int k = 0, u = 0;
                        ...
                        if (Back == 1)
                        {
                            ...
                            {
                                img = MergeTwo8bpp(img, img2);
                            }
                            else
                            img = MergeTwo32Argb(img, img2);
                        }
                        int width = img.Width;
                        ...
                        if ((Back == 1))
                        {
                            ...
                            if (tileheight2 > 0)
                            {
                                ...
                                if (Num == 33)
                                {
                                    byte[] a1 = new byte[NewTile.Length];
                                    for (int h = 0; h < NewTile.Length / 32; h++)
                                        for (int d = 0; d < 32; d++)
                                            a1[(h * 32) + d] = NewTile[h, d];
                                    File.WriteAllBytes("Test.bin", a1);
                                }
            ...
            File.WriteAllBytes("testNew.gba", memblock);
        }
    }
}